<template>
  <div
    class="bg-surface border border-border rounded-2xl shadow-sm p-6 transition-all duration-300 hover:shadow-md"
    role="region"
    aria-label="Smart weather insights"
  >
    <div class="flex items-center justify-between mb-4">
      <div>
        <h3 class="text-lg font-bold text-text-main tracking-tight">24-Hour Weather Summary</h3>
        <p class="text-text-light mt-2">Analyzing recent conditions & trends</p>
      </div>
    ...
    <div v-if="isSummaryLoading" class="text-text-light">
      <div class="animate-pulse space-y-3">
        <div class="h-24 bg-hover rounded-lg"></div>
        <div v-for="i in 3" :key="i" class="flex justify-between items-center">
          <div class="h-4 w-24 bg-hover rounded"></div>
          <div class="h-4 w-16 bg-hover rounded"></div>
        </div>
      </div>
    </div>

    <div v-else-if="summaryData?.error" class="flex items-center space-x-3 text-red-600">
    ...
    <div v-else-if="summaryData" class="space-y-4">
      ...
      <div v-if="summaryData.pattern" class="p-3 border rounded-lg bg-blue-50 border-blue-200">
        <div class="flex items-start space-x-2">
          <Icon icon="ph:trend-up-bold" class="h-5 w-5 text-blue-600 mt-0.5" />
          <div class="flex-1">
            <p class="text-sm font-medium text-blue-900">Observed Pattern</p>
            <p class="text-xs mt-1 text-blue-700">{{ summaryData.pattern }}</p>
          </div>
        </div>
      </div>
      ...
      <div class="space-y-2">
        ...
        <div
          ...
          :key="key"
          class="flex justify-between items-center text-sm py-2.5 border-b border-border last:border-0"
        >
      ...
      >
        <div class="flex items-start space-x-2">
          <Icon icon="ph:info-bold" class="h-5 w-5 text-purple-600 mt-0.5" />
          <div class="flex-1">
            <p class="text-sm font-medium text-purple-900">Advisory</p>
      ...
      <div class="flex items-start space-x-2 p-2 rounded text-xs text-text-light bg-hover">
    ...
    <div v-else class="text-center py-8">
      ...
      <p class="text-sm font-semibold mb-1 text-text-main">Collecting data...</p>
